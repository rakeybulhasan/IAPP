...
$admin_quicklink_support_buttons = ( current_user_can( $capabilities_whitelabel ) ) ? "<a class='" . $buttonclass7 ."' href='" . LEAFLET_WP_ADMIN_URL . "admin.php?page=leafletmapsmarker_help'><img src='" . LEAFLET_PLUGIN_URL . "inc/img/icon-menu-help.png' width='10' height='10' /> ".__('Support', 'lmm')."</a>&nbsp;&nbsp;&nbsp; &nbsp;&nbsp;&nbsp;" : "";
$admin_quicklink_license_buttons = ( current_user_can( $capabilities_whitelabel ) ) ? "<a class='" . $buttonclass8 ."' href='" . LEAFLET_WP_ADMIN_URL . "admin.php?page=leafletmapsmarker_license'><img src='" . LEAFLET_PLUGIN_URL . "inc/img/icon-menu-settings.png' width='10' height='10' /> ".__('License settings', 'lmm')."</a>" : "";
...
//info: display warning if WordPress <3.3 is used
global $wp_version;
if ( version_compare( $wp_version, '3.3', '<' ) ) {
	echo '<p><div class="error" style="padding:10px;margin:10px 0;">' . sprintf(__('<strong>Warning: you are using the outdated WordPress version %s !</strong><br/>Not all features of Leaflet Maps Marker Pro (like tracks) will work properly with this version. An update of your WordPress installation is highly recommended!','lmm'), $wp_version) . '</div></p>';
}
...
if ( lmm_pro_version_info() ) {
	...
	if (!is_multisite()) {
		if ( (file_exists($lmm_free_readme)) && (lmm_validate_license_info($release_date_info=false, $license_only_info=true)===true) ) {
				if ($free_version_metadata['Version'] >= '3.6') {
...
if ( $error_message == null ) { //info: dont show if get error
	if ( lmm_validate_license_info() ) {
		...
		if ( $latest_pro_version_transient === FALSE ) {
			$latest_pro_version_array = wp_remote_post('http://www.mapsmarker.com/version.json');
			...
			} else { //info: fallback if unreachable
		...
		if ($page != 'leafletmapsmarker_license') {
			echo "<div id='message' class='error' style='padding:10px;margin:10px 0;'><strong>" . __('Warning: your access to updates and support for Leaflet Maps Marker Pro has expired!','lmm') . "</strong><br/>";
			if ($latest_pro_version > $plugin_version) {
				echo __('Latest available version:','lmm') . " <a href='http://www.mapsmarker.com/v" . $latest_pro_version . "p' target='_blank' title='" . esc_attr__('click to show release notes','lmm') . "'>" . $latest_pro_version . "</a> " . "(<a href='www.mapsmarker.com/changelog/pro/' target='_blank'>" . __('show all available changelogs','lmm') . "</a>)<br/>";
			}
...
<table cellpadding="5" cellspacing="0" class="widefat fixed">
  <tr>
    <td><div style="float:left;margin:7px 10px 0 0;<?php echo $css_whitelabel = ($lmm_options['misc_whitelabel_backend'] == 'enabled') ? 'display:none' : '' ?>"><a href="http://www.mapsmarker.com/go" target="_blank" title="www.mapsmarker.com"><img src="<?php echo LEAFLET_PLUGIN_URL ?>inc/img/logo-mapsmarker-pro.png" width="65" height="65" alt="Leaflet Maps Marker Plugin Logo by Julia Loew, www.weiderand.net" /></a></div>
<?php
if ($lmm_options['misc_whitelabel_backend'] == 'disabled') {
	...
	if ( $lmm_options['misc_betatest'] == 'disabled' ) {
		echo '<div style="font-size:1.5em;margin-bottom:5px;"><span style="font-weight:bold;">Maps Marker<sup style="font-size:75%;">&reg;</sup> <a href="http://www.mapsmarker.com/v' . $pro_version['Version'] . 'p" target="_blank" title="' . esc_attr__('view blogpost for current version','lmm') . '">v' . $pro_version['Version'] . '</a> - ' .  __('Pro Edition','lmm') . '</span>';
	} else {
		echo '<div style="font-size:1.5em;margin-bottom:5px;padding:2px 0 0 0;">v' . $pro_version['Version'] . ' - ' .  __('Pro Edition','lmm') . '</span>';
	}
} else {
...
if (get_option('leafletmapsmarker_update_info') == 'show') {
	...
	$lmm_changelog_new_version = '<a href="http://www.mapsmarker.com/v' . $lmm_version_new . 'p" target="_blank" style="text-decoration:none;">http://www.mapsmarker.com/v' . $lmm_version_new . 'p</a>';
