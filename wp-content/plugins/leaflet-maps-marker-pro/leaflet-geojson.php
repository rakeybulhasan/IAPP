...
if (isset($_GET['layer'])) {
  ...
  if ($full == 0) {
	 $sql = 'SELECT m.id as mid, m.lat as mlat, m.lon as mlon, m.markername as mmarkername, m.icon as micon, m.popuptext as mpopuptext, m.address as maddress FROM '.$table_name_markers.' AS m '.$q;
  } else {
  ...
  if ($layer != NULL) { //info: needed for adding new layers
	  foreach ($markers as $marker) {
		...
		if ( ($full == 0) && ($lmm_options['directions_popuptext_panel'] == 'yes') ) {
			...
			echo '"address":'.json_encode($maddress).','.PHP_EOL;
		}
		...
		if ( ($lmm_options['directions_popuptext_panel'] == 'yes') || ($full == 1) ) {
			...
			if ($lmm_options['directions_provider'] == 'googlemaps') {
				...
				$directionslink = "http://map.project-osrm.org/?hl=" . $lmm_options[ 'directions_osrm_language' ] . "&loc=" . $marker['mlat'] . "," . $marker['mlon'] . "&df=" . $lmm_options[ 'directions_osrm_units' ];
			} else if ($lmm_options['directions_provider'] == 'ors') {
				$directionslink = "http://openrouteservice.org/index.php?end=" . $marker['mlon'] . "," . $marker['mlat'] . "&pref=" . $lmm_options[ 'directions_ors_route_preferences' ] . "&lang=" . $lmm_options[ 'directions_ors_language' ] . "&noMotorways=" . $lmm_options[ 'directions_ors_no_motorways' ] . "&noTollways=" . $lmm_options[ 'directions_ors_no_tollways' ];
