...
class optionsModelGmp extends modelGmp {
	...
	public function saveBgImg($d = array()) {
		if(!empty($d) && isset($d['bg_image']) && !empty($d['bg_image'])) {
			...
			if($uploader->validate('bg_image', frameGmp::_()->getModule('options')->getBgImgDir()) && $uploader->upload()) {
				// Remove prev. image
	...
	public function saveLogoImg($d = array()) {
		if(!empty($d) && isset($d['logo_image']) && !empty($d['logo_image'])) {
			$uploader = toeCreateObjGmp('fileuploader', array());
			if($uploader->validate('logo_image', frameGmp::_()->getModule('options')->getLogoImgDir()) && $uploader->upload()) {
				// Remove prev. image
	...
	public function saveFavico($d = array()) {
		...
		if(!empty($d) && isset($d['favico']) && !empty($d['favico']['name'])) {
			...
			$valid =$uploader->validate('favico', frameGmp::_()->getModule('options')->getFavicoDir(),'favico.png');
			utilsGmp::deleteFile(frameGmp::_()->getModule('options')->getFavicoFullDir());
	...
	public function setTplAnyDefault($d = array()) {
		...
		switch($code) {
			...
			case 'slider_images':
				$newOptValue = false;
				if(frameGmp::_()->getModule('bg_slider')) {
					if(frameGmp::_()->getModule('bg_slider')->getController()->getModel()->saveTplDefaultSlides()) {
						$newOptValue = array(
							'slides' => frameGmp::_()->getModule('bg_slider')->getSlidesFullPath(),
							'slidesNames' => frameGmp::_()->getModule('options')->get('slider_images'),
						);
					} else
						$this->pushError( frameGmp::_()->getModule('bg_slider')->getController()->getModel()->getErrors() );
				} else {
					$this->pushError(langGmp::_('There are no Bg slider module installed'));
				}
				break;
			default:
