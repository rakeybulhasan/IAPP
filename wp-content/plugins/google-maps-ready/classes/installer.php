...
class installerGmp {
	...
	static public function init() {
		...
		if (!dbGmp::exist($wpPrefix.GMP_DB_PREF."htmltype")) {
			$q = "CREATE TABLE IF NOT EXISTS `".$wpPrefix.GMP_DB_PREF."htmltype` (
		...
		dbGmp::query("INSERT INTO `".$wpPrefix.GMP_DB_PREF."htmltype` VALUES
			...
			(18, 'checkboxHiddenVal', 'Checkbox with Hidden field')");
                        // var_dump(dbGmp::exist($wpPrefix.GMP_DB_PREF."modules"));
                        //exit("trololo");
                    }
		/**
		 * modules 
		...
		if (!dbGmp::exist($wpPrefix.GMP_DB_PREF."modules")) {
			$q = "CREATE TABLE IF NOT EXISTS `".$wpPrefix.GMP_DB_PREF."modules` (
			  `id` int(11) NOT NULL AUTO_INCREMENT,
		...
		dbGmp::query("INSERT INTO `".$wpPrefix.GMP_DB_PREF."modules` (id, code, active,    
		...
		if(!dbGmp::exist($wpPrefix.GMP_DB_PREF."modules_type")) {
			$q = "CREATE TABLE IF NOT EXISTS `".$wpPrefix.GMP_DB_PREF."modules_type` (
			  `id` int(11) NOT NULL AUTO_INCREMENT,
		...
		if(!dbGmp::exist($wpPrefix.GMP_DB_PREF."options")) {
			$q = "CREATE TABLE IF NOT EXISTS `".$wpPrefix.GMP_DB_PREF."options` (
			  `id` int(11) NOT NULL AUTO_INCREMENT,
		...
		if(!dbGmp::exist($wpPrefix.GMP_DB_PREF."options_categories")) {
			$q = "CREATE TABLE IF NOT EXISTS `".$wpPrefix.GMP_DB_PREF."options_categories` (
			  `id` int(11) NOT NULL AUTO_INCREMENT,
	...
	static public function update() {
		...
		if(!$currentVersion || version_compare(GMP_VERSION, $currentVersion, '>')) {
			self::init();
			update_option($wpPrefix. 'db_version', GMP_VERSION);
		}
	}
}
