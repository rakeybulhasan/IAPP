    ...
    static public function radiobuttons($name, $params = array('attrs' => '', 'options' => array(), 'value' => '', '')) {
		...
		if(isset($params['options']) && is_array($params['options']) && !empty($params['options'])) {
			...
			$params['mo_br'] = isset($params['mo_br']) ? $params['mo_br'] : false;
                        $labelClass=isset($params['labelClass'])?$params['labelClass']:false;
			foreach($params['options'] as $key => $val) {
				$id = self::nameToClassId($key). '_'. mt_rand(1, 999999);
				$attrs = $params['attrs'];
				$attrs .= ' id="'. $id. '"';
				$checked =($key == $params['value']) ? 'checked' : '';
				$label = $params['labeled'] ? '<label class="'.$labelClass.'" for="'. $id. '">'. $val. '</label>' : $val;
				$out .= self::input($name, array('attrs' => $attrs.' '.$checked, 'type' => 'radio', 'value' => $key)). ' '. $label. ($params['mo_br'] ? '' : '<br class="gmpRadioBr" />');
			}
		}
        return $out;
    }
        ...
        if(!empty($params['options'])) {
            ...
            do {
                ...
                foreach($params['options'] as $key => $p) {
                    ...
                    $res .= langGmp::_($p['label']). htmlGmp::text($name. '['. $i. ']['. $key. ']', $paramsForText);
                }
                $res .= $remove. '</div>';
                $i++;
