...
if ( $customer_orders ) : ?>
	...
	<div class="headline" style="margin-top: 5px;"><h3><?php echo apply_filters( 'woocommerce_my_account_my_orders_title', __( 'Recent Orders', 'woocommerce' ) ); ?></h3></div>

	<table class="standard-table my_account_orders">
					...
					<td class="order-actions" style="text-align:right; white-space:nowrap;">
						<?php
							...
							foreach( $actions as $action ) {
								echo '<a href="' . esc_url( $action['url'] ) . '" class="button color ' . sanitize_title( $action['name'] ) . '">' . esc_html( $action['name'] ) . '</a>';
							}
						?>
					</td>
				</tr><?php
